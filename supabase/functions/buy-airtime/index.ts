...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { phone_number, amount, network } = await req.json();

    if (!phone_number || !amount || amount < 5 || !network) {
    ...
    const { data: reference } = await supabaseAdmin.rpc("generate_transaction_reference", { prefix: "AIR" });
    ...
    const { data: tx } = await supabaseAdmin.from("transactions").insert({
      ...
      phone_number,
      description: `${network} airtime to ${phone_number}`,
      status: "pending",
      metadata: { network },
    }).select().single();
    ...
    await supabaseAdmin.from("ledger").insert({
      ...
      description: `Airtime ${network} to ${phone_number}`,
    });

    // TODO: Call actual airtime API here
